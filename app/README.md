...
4. Create a private environment file owned by that account, mode `0600`, with values based on [`.env.example`](.env.example). Put a random `ADMIN_TOKEN` and the hosted model API key there. Do not put secrets in systemd unit text, Git, shell history, or the PWA.
...
7. Add a `/dwh/api/*` route to the existing private Caddy listener that strips `/dwh`, proxies to `127.0.0.1:4387`, and sets `X-Admin-Token` to the server's admin token. Add `{"id":"dwh","name":"Bank DWH Studio","api":"/dwh"}` to the invite console's `apps.json`. The console's route must remain tailnet-only; the app's admin endpoints return 404 without the token.
