...
The app checks for PWA updates at startup, on foreground return, and every minute while visible. It saves the current question, SQL, and subject area in the current tab's session storage so the kit can reload after an update without discarding a draft. An in-progress request still delays the reload until it finishes. If a device is stuck on code from before this behavior was deployed, open `/bust` once to clear its cached app shell and re-register the service worker.
